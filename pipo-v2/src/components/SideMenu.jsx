...
import { Menu, X, MessageSquare, MapPin, Calendar, Heart, Settings, BookOpen } from 'lucide-react'
  ...
  return (
    <>
      ...
      >
          ...
          <nav className="py-3 sm:py-4 flex-1 overflow-y-auto">
            {MENU_ITEMS.map(({ id, label, icon: Icon }) => {
              const isCurrent = (id === 'cita')
                ...
                : (id === 'ajustes' || id === 'directorio' ? false : currentView === id)
        ...
        {showDirectorio && (
          <DirectorioPanel onBack={() => setShowDirectorio(false)} />
        )}
      </aside>
    </>
  )
